...
public partial class GameStorage {
    public partial class Request : IPlayerInfoProvider {
        ...
        public bool SavePlayer(Player player) {
            ...
            // Ensure tracking is enabled for this save operation
            // var prevBehavior = Context.ChangeTracker.QueryTrackingBehavior;
            // Context.ChangeTracker.QueryTrackingBehavior = QueryTrackingBehavior.TrackAll;
            // try {
            //     // Fetch tracked entities (ensures concurrency token original values are correct)
            //     Model.Account? dbAccount = Context.Account.Find(player.Account.Id);
            //     if (dbAccount == null) {
            //         Console.WriteLine($"> SavePlayer abort: Account {player.Account.Id} not found");
            //         return false;
            //     }
            //     Model.Character? dbCharacter = Context.Character.Find(player.Character.Id);
            //     if (dbCharacter == null || dbCharacter.AccountId != player.Account.Id) {
            //         Console.WriteLine($"> SavePlayer abort: Character {player.Character.Id} not found or mismatched account");
            //         return false;
            //     }
            //     CharacterUnlock? dbUnlock = Context.CharacterUnlock.Find(player.Character.Id);
            //     if (dbUnlock == null) {
            //         Console.WriteLine($"> SavePlayer abort: CharacterUnlock {player.Character.Id} not found");
            //         return false;
            //     }
            //
            //     // If any entity is still detached (shouldn't happen with Find), attach them.
            //     if (Context.Entry(dbAccount).State == EntityState.Detached) {
            //         Context.Attach(dbAccount);
            //         Console.WriteLine("> Diagnostic: Account entity was detached; manually attached.");
            //     }
            //     if (Context.Entry(dbCharacter).State == EntityState.Detached) {
            //         Context.Attach(dbCharacter);
            //         Console.WriteLine("> Diagnostic: Character entity was detached; manually attached.");
            //     }
            //     if (Context.Entry(dbUnlock).State == EntityState.Detached) {
            //         Context.Attach(dbUnlock);
            //         Console.WriteLine("> Diagnostic: CharacterUnlock entity was detached; manually attached.");
            //     }
            //
            //     // Update Account fields (only those that can change during a session)
            //     var srcAccount = player.Account;
            //     dbAccount.Online = srcAccount.Online;
            //     dbAccount.PremiumTime = srcAccount.PremiumTime;
            //     dbAccount.PrestigeLevel = srcAccount.PrestigeLevel;
            //     dbAccount.PrestigeLevelsGained = srcAccount.PrestigeLevelsGained;
            //     dbAccount.PrestigeExp = srcAccount.PrestigeExp;
            //     dbAccount.PrestigeCurrentExp = srcAccount.PrestigeCurrentExp;
            //     dbAccount.PrestigeRewardsClaimed = srcAccount.PrestigeRewardsClaimed;
            //     dbAccount.PrestigeMissions = srcAccount.PrestigeMissions.Select(m => new Maple2.Database.Model.PrestigeMission {
            //         Id = m.Id,
            //         GainedLevels = m.GainedLevels,
            //         Awarded = m.Awarded
            //     }).ToList();
            //     dbAccount.PremiumRewardsClaimed = srcAccount.PremiumRewardsClaimed;
            //     dbAccount.ActiveGoldPass = srcAccount.ActiveGoldPass;
            //     dbAccount.SurvivalLevel = srcAccount.SurvivalLevel;
            //     dbAccount.SurvivalExp = srcAccount.SurvivalExp;
            //     dbAccount.SurvivalSilverLevelRewardClaimed = srcAccount.SurvivalSilverLevelRewardClaimed;
            //     dbAccount.SurvivalGoldLevelRewardClaimed = srcAccount.SurvivalGoldLevelRewardClaimed;
            //     dbAccount.Currency = new AccountCurrency {
            //         Meret = player.Currency.Meret,
            //         GameMeret = player.Currency.GameMeret,
            //         MesoToken = player.Currency.MesoToken,
            //     };
            //     dbAccount.MarketLimits.MesoListed = srcAccount.MesoMarketListed;
            //     dbAccount.MarketLimits.MesoPurchased = srcAccount.MesoMarketPurchased;
            //
            //     // Update Character fields
            //     var srcCharacter = player.Character;
            //     dbCharacter.Level = srcCharacter.Level;
            //     dbCharacter.MapId = srcCharacter.MapId;
            //     dbCharacter.ReturnMapId = srcCharacter.ReturnMaps.Peek();
            //     dbCharacter.Channel = srcCharacter.Channel;
            //     dbCharacter.ReturnChannel = srcCharacter.ReturnChannel;
            //     dbCharacter.MentorRole = srcCharacter.MentorRole;
            //     dbCharacter.DeleteTime = srcCharacter.DeleteTime.FromEpochSeconds();
            //     dbCharacter.Experience.Exp = srcCharacter.Exp;
            //     dbCharacter.Experience.RestExp = srcCharacter.RestExp;
            //     dbCharacter.Profile.Motto = srcCharacter.Motto;
            //     dbCharacter.Profile.Picture = srcCharacter.Picture;
            //     dbCharacter.Profile.Title = srcCharacter.Title;
            //     dbCharacter.Profile.Insignia = srcCharacter.Insignia;
            //     dbCharacter.Cooldown.Doctor = srcCharacter.DoctorCooldown;
            //     dbCharacter.Cooldown.Storage = srcCharacter.StorageCooldown;
            //     dbCharacter.Mastery.Alchemy = srcCharacter.Mastery.Alchemy;
            //     dbCharacter.Mastery.Cooking = srcCharacter.Mastery.Cooking;
            //     dbCharacter.Mastery.Farming = srcCharacter.Mastery.Farming;
            //     dbCharacter.Mastery.Fishing = srcCharacter.Mastery.Fishing;
            //     dbCharacter.Mastery.Foraging = srcCharacter.Mastery.Foraging;
            //     dbCharacter.Mastery.Handicrafts = srcCharacter.Mastery.Handicrafts;
            //     dbCharacter.Mastery.Smithing = srcCharacter.Mastery.Smithing;
            //     dbCharacter.Mastery.Instrument = srcCharacter.Mastery.Instrument;
            //     dbCharacter.Mastery.Mining = srcCharacter.Mastery.Mining;
            //     dbCharacter.Mastery.PetTaming = srcCharacter.Mastery.PetTaming;
            //     dbCharacter.Mastery.Ranching = srcCharacter.Mastery.Ranching;
            //     dbCharacter.Currency = new CharacterCurrency {
            ...
            //
            //     // Unlock
            //     Unlock srcUnlock = player.Unlock;
            //     dbUnlock.Titles = srcUnlock.Titles;
            //     dbUnlock.InteractedObjects = srcUnlock.InteractedObjects;
            //     dbUnlock.FishAlbum = srcUnlock.FishAlbum.Values.Select(f => (Maple2.Database.Model.FishEntry) f).ToList();
            //     dbUnlock.MasteryRewardsClaimed = srcUnlock.MasteryRewardsClaimed;
            //     dbUnlock.Emotes = srcUnlock.Emotes;
            //     dbUnlock.Pets = srcUnlock.Pets;
            //
            //     Context.ChangeTracker.Entries().DisplayStates();
            //     try {
            //         bool result = Context.TrySaveChanges();
            //         Console.WriteLine(result ? $"> Save committed {Context.ContextId}" : $"> Save failed (see earlier error) {Context.ContextId}");
            //         return result;
            //     } finally {
            //         Console.WriteLine($"> End Save. {Context.ContextId}");
            //     }
            // } finally {
            //     // restore previous behavior
            //     Context.ChangeTracker.QueryTrackingBehavior = prevBehavior;
            // }
